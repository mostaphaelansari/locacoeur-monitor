...
# Email settings - Fixed configuration
EMAIL_CONFIG = {
...
class MQTTService:
    ...
    def send_alert_email(self, subject: str, message: str, priority: str = "normal"):
        """Send email alert with rate limiting - Fixed email configuration"""
...
"""
    ...
    def detect_critical_alerts(self, device_serial: str, topic: str, data: Dict[str, Any]) -> None:
        ...
        alerts = []
        critical_alert_ids = [5, 6, 7]  # Power cut, Critical temperature, Defibrillator fault

        if "alert" in topic.lower():
            alert_id = data.get("id")
            alert_message = ALERT_CODES.get(alert_id, f"Unknown alert {alert_id}")

            if alert_id in critical_alert_ids:
                alerts.append(f"Critical alert: {alert_message} (ID: {alert_id})")
    ...
    def insert_device_data(self, device_serial: str, topic: str, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Handle alert events
                ...
                alert_id = data.get("id")
                alert_message = data.get("message", ALERT_CODES.get(alert_id, f"Unknown alert {alert_id}"))
                ...
                self.send_alert_email(
                    f"Device Alert - {device_serial}",
                    f"Alert for device {device_serial}: {alert_desc} (ID: {alert_id})",
                    "critical" if alert_id in [5, 6, 7] else "warning"
                )
